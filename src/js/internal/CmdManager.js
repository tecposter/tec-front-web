...
export class CmdManager {
    constructor() {
        ...
        document.on(DocumentEvent.keydown, async evt => {
            ...
            const cmdName = this.shortKeys[shortKey];
            if (!cmdName) {
                return;
            }
    ...
    trigger(cmdName) {
        if (!this.cmds.hasOwnProperty(cmdName)) {
            console.warn(`cannot find cmdName: ${this.mode} - ${cmdName}`); // eslint-disable-line no-console
            return;
            //throw new Error(`Cannot find cmdName: ${this.mode} - ${cmdName}`);
        }
        const cmd = this.cmds[cmdName];
        cmd.fun.apply(null);
    }

    addCmd(cmdName, cmdDesc, cmdShortKeys, cmdFun) {
        if (this.cmds.hasOwnProperty(cmdName)) {
            throw new Error(`duplicated cmdName: ${cmdName}`);
        }
        this.cmds[cmdName] = {key: cmdName, desc: cmdDesc, shortKeys: cmdShortKeys || '', fun: cmdFun};
        if (cmdShortKeys) {
            cmdShortKeys.split(',').forEach(shortKey => this.mapShortKey(shortKey, cmdName));
        }
    }
